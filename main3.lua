...
-- THÊM NÚT LOCK CÓ CHỮ VÀO TOPBAR
...
LockFrame.Size = UDim2.new(0, 60, 0, 15)
...
LockText.Text = "Lock UI"
LockText.TextColor3 = Color3.fromRGB(255,255,255)
LockText.TextSize = 11
LockText.TextTransparency = 0.5

-- BIẾN THEO DÕI TRẠNG THÁI LOCK (phải khai báo trước các hàm draggable)
local isLocked = false

-- ========== SỬA HÀM DRAGGABLE ==========
-- Tìm hàm Draggable trong Library và sửa lại
local originalDraggable = Library.Draggable
...
function Library:Draggable(inputframe, dragframe)
    ...
    inputframe.InputBegan:Connect(function(input)
        -- KIỂM TRA TRẠNG THÁI LOCK TRƯỚC KHI CHO PHÉP KÉO
    ...
    UserInputService.InputChanged:Connect(function(input)
        -- KIỂM TRA TRẠNG THÁI LOCK TRƯỚC KHI DI CHUYỂN
        if isLocked then return end
...
local function toggleLock()
    ...
    if isLocked then
        -- Đổi chữ thành "Open UI"
        LockText.Text = "Open UI"
        Library:Tween({
            v = LockText,
            t = 0.2,
            s = "Exponential",
            d = "Out",
            g = {TextTransparency = 0, TextColor3 = Color3.fromRGB(255, 100, 100)}
        }):Play()
        ...
        Library:Tween({
            ...
            s = "Exponential",
            d = "Out", 
            g = {TextTransparency = 0.5, TextColor3 = Color3.fromRGB(255, 255, 255)}
        }):Play()
